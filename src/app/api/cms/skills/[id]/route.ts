...
export async function GET(request: NextRequest, { params }: slugParams) {
    try {
        ...
        const docRef = doc(db, "cms_skills", id)
        const querySnapshot = await getDoc(docRef);
        let tempArr = null
        if (querySnapshot.exists()) {
            tempArr = querySnapshot.data()
        } else {
            return Response.json({ message: 'No such skills content!', status: "FAILURE"})
        }
...
export async function PATCH(request: NextRequest, { params }: slugParams) {
    try {
        ...
        return Response.json({ message: 'successfully updated the skill content', status: "SUCCESS" })
    } catch (error: any) {
        return Response.json({
            status: 'FAILURE',
            message: (error as Error).message
...
export async function DELETE(request: NextRequest, { params }: slugParams) {
    try {
        const id = (await params).id
        await deleteDoc(doc(db, "cms_skills", id));
        return Response.json({ message: 'successfully deleted a skill content', status: "SUCCESS" })
    } catch (error: any) {
        return Response.json({
            status: 'FAILURE',
            message: (error as Error).message
