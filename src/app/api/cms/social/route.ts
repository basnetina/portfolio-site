...
import {SocialType, SocialTypeGet} from "@/types/socialType";

export async function GET(req: NextRequest){
    try {
        let querySnapshot;
        const qLanding = query(
            collection(db, "cms_social")
        );
        ...
        const socialCMS: SocialTypeGet[] = []
        querySnapshot.forEach((doc: { id: any; data: () => any; }) => {
            socialCMS.push({...doc.data(), id: doc.id})
        });

        return Response.json({
            status: 'SUCCESS',
            message: 'Data retrieved Successfully!',
            cms_social: socialCMS
        })
    } catch (e) {
        return Response.json({
            status: 'FAILURE',
...
export async function POST(req: NextRequest){
    try {
        ...
        const requestData: SocialType[] = await req.json();
        ...
        const newRef = await addDoc(collection(db, "cms_social"), postData)
